...
import { ObjectQueue } from '../../DataStructure/Queue';

type ResultTyp<E> = { 
  elementList: (E | undefined)[]; 
  winner: E | undefined;
};

export function drummingSpreadFlower<E>(list: E[], num: number): ResultTyp<E> {
  const queue = new ObjectQueue<E>();
  const elementList = [];
  ...
  while(queue.getSize() > 1) {
    for (let j = 0; j < num; j++) {
      queue.enqueue(queue.dequeue() as E); // 循环队列操作
    }
